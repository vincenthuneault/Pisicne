...
- **`LiaisonArduino`** — encapsule `pyserial` ; un thread de lecture dédié (arrêtable) dépile les lignes JSON et place les événements (`{"event":"bouton",...}`) dans une `queue.Queue`, en routant les réponses d'état des boutons à part ; expose une méthode par commande primitive (`ouvrir_valve`, `fermer_valve`, `arreter_valve`, `demarrer_moteur`, `arreter_moteur`, `lire_etat_boutons`) ainsi que `fermer()`/`rouvrir()` pour libérer le port le temps du flashage. Un mode **simulation** (`PISCINE_SIMULATION=1`) permet de tester sans Arduino.
