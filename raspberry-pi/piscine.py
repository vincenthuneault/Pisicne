...
class LiaisonArduino:
    ...
    Le port peut être fermé puis rouvert (fermer()/rouvrir()) pour libérer
    /dev/ttyACM0 le temps de reprogrammer l'Arduino (voir flashage.py).
    ...
    def lire_etat_boutons(self, timeout=2):
        ...
        Renvoie None si aucune réponse n'arrive (ou en simulation).
        """
        if self.simulation:
            return {"vert": False, "rouge": False}
        ...
        try:
            return self._reponses_boutons.get(timeout=timeout)
        except queue.Empty:
            return None
...
# est pressé en cours de route. Les étapes reproduisent exactement celles
# décrites dans la documentation Obsidian.

def sequence_demarrage(arduino, urgence, config):
    """Bouton vert : ouvrir toutes les valves, puis démarrer le moteur."""
...
def sequence_demarrage_avec_priming(arduino, urgence, config):
    ...
    # 1) Écumoire et alimentation grandes ouvertes pendant l'amorçage initial
    ...
    # 3) Une fois le retour complètement ouvert : démarrage du moteur
    arduino.demarrer_moteur()

    # 4) Stabilisation, moteur en marche
    urgence.attendre(p["stabilisation_s"])

    # 5) Priming de la ligne de l'écumoire : cycles d'impulsion
    #    ouverture partielle (~5 %) puis fermeture immédiate
    for _ in range(p["nb_cycles"]):
        arduino.ouvrir_valve("ecumoire", p["impulsion_ouverture_ms"])
        urgence.attendre(p["impulsion_ouverture_ms"] / 1000)
        arduino.fermer_valve("ecumoire", p["impulsion_fermeture_ms"])
        urgence.attendre(p["impulsion_fermeture_ms"] / 1000)

    # 6) Ouverture complète de l'écumoire — système fonctionnel
    arduino.ouvrir_valve("ecumoire", course_ms)
    urgence.attendre(course_ms / 1000)
...
class Orchestrateur:
    ...
    def lire_boutons(self):
        with self._verrou_commande:
            if self._flashage:
                return None
        return self.arduino.lire_etat_boutons()
